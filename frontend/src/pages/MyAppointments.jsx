    ...
    try {
      ...
      const options = {
        ...
        handler: async (response) => {
          try {
            const { data: v } = await axios.post(`${backendUrl}/api/user/verify-razorpay`, response, { headers: { token } })
            if (v.success) { toast.success('Payment successful! 🎉'); getUserAppointments() }
    ...
    <div className="max-w-4xl mx-auto px-6 py-12">
      ...
      {loading ? (
        ...
        <div className="text-center py-20 bg-white rounded-3xl border border-slate-100">
          ...
          <button onClick={() => navigate('/doctors')} className="bg-gradient-to-r from-sky-500 to-cyan-500 text-white px-6 py-3 rounded-xl font-semibold">Find Doctors</button>
        </div>
      ) : (
        <div className="space-y-4">
            ...
            <div key={apt._id} className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden hover:shadow-md transition-shadow">
              <div className="flex flex-col sm:flex-row">
                <div className="sm:w-32 bg-gradient-to-br from-sky-50 to-cyan-50 flex items-center justify-center p-4">
                  <img src={apt.docData?.image} alt={apt.docData?.name} className="w-24 h-24 rounded-2xl object-cover"
                ...
                <div className="flex-1 p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  ...
                  <div className="flex flex-col gap-2 min-w-[140px]">
                    {!apt.cancelled && !apt.isCompleted && !apt.payment && (
                      <button onClick={() => payWithRazorpay(apt._id)} disabled={payingId === apt._id}
                        className="flex items-center justify-center gap-2 bg-gradient-to-r from-sky-500 to-cyan-500 text-white px-4 py-2.5 rounded-xl text-sm font-semibold hover:shadow-md transition-all disabled:opacity-60">
                        {payingId === apt._id
